	...
	public static List<Jogador> getJogadoresOnline() {
		...
		if(objBD.abrirConexao())
		{
			try
			{
				//Foi utilizado a classe StringBuilder, que nesse caso realiza a mesma função do que uma
				//concatenação de strings
				StringBuilder strSQL = new StringBuilder("SELECT cod_id_jogador, SEAWAR.JOGADOR.cod_id_usuario, status_jogador, ip_jogador, SEAWAR.USUARIO.login FROM SEAWAR.JOGADOR");
				strSQL.append(" INNER JOIN SEAWAR.USUARIO ON SEAWAR.USUARIO.cod_id_usuario = SEAWAR.JOGADOR.cod_id_usuario");
				strSQL.append(" WHERE status_jogador = 1 and status_usuario = 1 and ip_jogador is not null;");
				ResultSet objRS = objBD.getObjStatement().executeQuery(strSQL.toString());
				while(objRS.next()){
					Jogador objJogador = new Jogador();
					//Verificar se precisa criar atributo para o ID_JOGADOR (objRS.getInt("COD_ID_JOGADOR"));
					objJogador.setId_usuario(objRS.getInt("COD_ID_USUARIO"));
					objJogador.setUsuarioAtivo(objRS.getBoolean("STATUS_JOGADOR"));

...
public class RankingDAO {
	...
	// Como esta classse não possui INSERT, DELETE ou UPDATE, só será feito o
	// SELECT
	...
	public static List<Ranking> consultarRanking() {
		...
		if (objBD.abrirConexao()) {
			try {
				ResultSet objRs = objBD
						.getObjStatement()
						.executeQuery(
								"SELECT CONCAT(@rownum := @rownum + 1,' º') AS POSICAO, r.login, r.pontuacao, r.cod_nacionalidade FROM seawar.ranking r, (SELECT @rownum := 0) p;");
				while (objRs.next()) {
					// Se tiver registros, instancia um tipo e atribui seus
					// valores de acordo
					...
					objRanking.setNacionalidade(Pais.getPais(objRs
							.getInt("COD_NACIONALIDADE")));

					// adiciona o tipo de mercadoria na lista de tipos de
					// mercadorias
		...
		if (objBD.abrirConexao()) {
			try {
				ResultSet objRs = objBD.getObjStatement().executeQuery(
						"SELECT COD_PAIS, NOME_PAIS,COD_ISO FROM SEAWAR.PAIS WHERE COD_PAIS = "
								+ strUsuario + ";");
				if (objRs.next()) {
					// Se tiver registros, instancia um tipo e atribui seus
					// valores de acordo

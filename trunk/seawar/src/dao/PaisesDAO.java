		...
		if (objBD.abrirConexao()) {
			try {
				ResultSet objRs = objBD
						.getObjStatement()
						.executeQuery(
								"SELECT COD_ID_PAIS, NOME_PAIS,COD_ISO FROM SEAWAR.PAIS;");
				while (objRs.next()) {
		...
		if (objBD.abrirConexao()) {
			try {
				ResultSet objRs = objBD.getObjStatement().executeQuery(
						"SELECT COD_ID_PAIS, NOME_PAIS,COD_ISO FROM SEAWAR.PAIS WHERE COD_ID_PAIS = "
								+ codigo + ";");
				if (objRs.next()) {
					// Se tiver registro, instancia um tipo e atribui seus
					// valores de acordo
					objPais.setPais(objRs.getInt("COD_ID_PAIS"));
					objPais.setNomePais(objRs.getString("NOME_PAIS"));
					objPais.setCodIso(objRs.getString("COD_ISO"));
